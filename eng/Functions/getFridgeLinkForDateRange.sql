...
drop table if exists #fridge
	...
	SELECT f.fridgeid,  u.link
	  FROM #fridge f
	 CROSS APPLY [Eng].[getFridgeLinkForDateRange] (f.fridgeid, '2020-04-27', 1200) u


*/
CREATE FUNCTION [Eng].[getFridgeLinkForDateRange] (@fridgeid bigint, @StartTime datetime2(0),@EndTime datetime2(0), @range int )  
...
begin  
   ...
   SELECT @result = 
         CASE WHEN (select top 1 1 from GroupFridge g where g.FridgeID = @fridgeid and GroupName = 'solar') = 1
              ...
              '&start=' + cast(DATEDIFF(SECOND, '1970-01-01', @StartTime) - (@range) as varchar) +
              '&stop=' + cast(DATEDIFF(SECOND, '1970-01-01', @EndTime) + (@range) as varchar)
